...
open class RemoteCreationControllerAPI {
    ...
    /**
     ...
     - parameter eduid: (path)  
     - returns: UpdateExternalEduID
     */
    @available(macOS 10.15, iOS 13.0, tvOS 13.0, watchOS 6.0, *)
    open class func deleteEduID(eduid: String) async throws -> UpdateExternalEduID {
        return try await deleteEduIDWithRequestBuilder(eduid: eduid).execute().body
    }
    ...
    /**
     ...
     - returns: RequestBuilder<UpdateExternalEduID> 
     */
    open class func deleteEduIDWithRequestBuilder(eduid: String) -> RequestBuilder<UpdateExternalEduID> {
        var localVariablePath = "/api/remote-creation/eduid-delete/{eduid}"
        let eduidPreEscape = "\(APIHelper.mapValueToPathItem(eduid))"
        let eduidPostEscape = eduidPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        localVariablePath = localVariablePath.replacingOccurrences(of: "{eduid}", with: eduidPostEscape, options: .literal, range: nil)
        let localVariableURLString = OpenAPIClientAPI.basePath + localVariablePath
