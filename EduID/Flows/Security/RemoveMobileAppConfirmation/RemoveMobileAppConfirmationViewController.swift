//
//  RemoveMobileAppConfirmationViewController.swift
//  eduID
//
...
class RemoveMobileAppConfirmationViewController: UIViewController, ScreenWithScreenType {
    
    // - screen type
    var screenType: ScreenType = .securityRemoveMobileAppConfirmation
    
    private let viewModel: RemoveMobileAppConfirmationViewModel
    ...
    init(viewModel: RemoveMobileAppConfirmationViewModel) {
    ...
    private func setupUI() {
        ...
        let titleString = L.ConfirmRemoveMobileApp.Title.localization
        ...
        let disclaimerString = NSMutableAttributedString(
            string: L.ConfirmRemoveMobileApp.Subtitle.localization,
        ...
        let descriptionLabel = UILabel.plainTextLabelPartlyBold(
            text: L.ConfirmRemoveMobileApp.Description.localization
        )
        ...
        cancelButton = EduIDButton(type: .ghost, buttonTitle: L.ConfirmRemoveMobileApp.Button.Cancel.localization)
        confirmButton = EduIDButton(type: .filledRed, buttonTitle: L.ConfirmRemoveMobileApp.Button.Confirm.localization)
        ...
        confirmButton.addTarget(self, action: #selector(sendCodeAndContinue), for: .touchUpInside)
    }
    
    @objc func sendCodeAndContinue() {
        Task {
            ...
            do {
                try await viewModel.sendDeactivationCode()
